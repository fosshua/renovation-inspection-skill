...
- 对近景、局部图、尺量图先评估证据完整度，避免把构件识别错或过度判断。
- 支持按“远景、标记位置、带尺近景、侧面/视频、施工记录”的证据包方式引导用户补拍。
...
- `references/evidence-capture-protocol.md`：图片、视频、尺量、样板和施工记录的证据采集规则。
...
默认输出采用“先结论、再行动、后依据”的分层结构，方便业主快速判断能不能继续施工、要不要整改、该怎么和施工方沟通。

该 skill 的输出通常包含：

- 结论：2-4 条短句，说明是否发现问题、风险等级、是否建议暂停下一工序、最紧急动作。
- 可以直接发给施工方：可复制的沟通话术。
- 施工阶段识别。
- 证据包完整度、测量可靠性和缺失的项目约定。
- 按严重程度排序的问题列表。
- 已检查项和未验证必检项。
- 每个问题对应的图片或视频证据编号。
- 证据强度和置信度。
...
对于正常图片、证据不足图片或图纸照片，输出会尽量简短，不会为了生成报告而硬判施工缺陷。

## 当前测试样例

- 窗框下口与地面/地砖收口缝隙偏大：可识别为门窗安装与窗边收口问题，命中“大缝隙用厚胶遮盖”模式，并提示不建议直接进入收口工序。
- 瓷砖海棠角宽度偏大：用户指出海棠角时，优先识别为贴砖与地面阶段的阳角/倒角问题，不误吸收到窗边缝隙。
